...
3. Codebase Review 2026-06-05 critical findings (Notion page `3769402f…`): paid-tier rate-limit/cost-cap key mismatch (`pro` vs `paid`); `/conversations/messages` bypasses rate limits and cost caps; sequential-blocking "parallel" retrieval.
...
- **Routing-node model — keep R0 (gemini-2.5-flash-lite); spike 15/06, decision = keep.** Revisit query-agent model (2.5→3.1) ONLY if prod traces show R0 is_reask false-positives degrading answers (conditional v13.5 injection failing to absorb a misfire on a genuine follow-up) OR a real re-ask missed. query_types disagreements are noise, not a trigger. Spike found R1 more precise on is_reask (10:1, n=1) but R0's imprecision is absorbed downstream — shipped B grid passed all affected scenarios; R1 not worth 3.5× on universal traffic. Scaffolding if the trigger fires: env-overridable `routing_model` hook (`ROUTING_MODEL`/`ROUTING_THINKING`, defaults to flash = unchanged) + harness on unmerged branch `eval/routing-node-spike` (results `routing_spike_*`).
